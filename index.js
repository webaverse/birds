...
const {useApp, useFrame, useLoaders, usePhysics, useCleanup, useRenderer} = metaversefile;
...
export default () => {
  ...
  const fragmentShaderVelocity = `\
      ...
      float zoneRadius = 150.0;
      float zoneRadiusSquared = 22500.0;
      ...
      void main() {
        ...
        // if (velocity.y > 0.) velocity.y *= (1. - 0.99 * delta);
  ...
  const _initBirds = () => {
    ...
    m.onBeforeCompile = ( shader ) => {
      ...
      shader.uniforms.size = { value: 0.02 };
    ...
    velocityUniforms[ "separationDistance" ].value = 2.0;
    velocityUniforms[ "alignmentDistance" ].value = 2.0;
    ...
    BirdGeometry.setDrawRange( 0, indicesPerBird * birdAmount );

  }

  const _initComputeRenderer = () => {
  ...
  let physicsIds = [];
  (async () => {
    const u = `${baseUrl}assets/Parrot.glb`;
